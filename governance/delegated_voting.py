...
def get_update_user_vebank_txns(
    client, sender, user_to_update, user_to_update_storage_address
):
...
def get_delegated_vote_txns(
    client, sender, voter, voter_storage_address, voter_delegating_to, proposal_app_id
):
    params = get_default_params(client.algod)
    ...
    for proposal_app_id in proposals:
        ...
        if is_proposal_open_for_voting:
            ...
            for governor_address in governor_state:
                delegating_to = governor_state[governor_address]["admin"][
                    "delegating_to"
                ]
                amount_vebank = (
                    client.governance.voting_escrow.get_projected_vebank_amount(
                        UserVotingEscrowState(
                            governor_state[governor_address]["voting_escrow"]
                        )
                    )
                )
                ...
                if delegating_to and amount_vebank > 0:
                    ...
                    user_open_to_delegation = governor_state[
                        storage_mapping[delegating_to]
                    ]["admin"]["open_to_delegation"]
                    ...
                    if (
                        user_open_to_delegation
                        and delegate_voted
                        and not governor_voted
                    ):
                        print(
                            "Voting for "
                            + governor_address
                            + " with "
                            + delegating_to
                            + " as delegate for proposal "
                            + str(proposal_app_id)
                        )
                        txn = get_delegated_vote_txns(
                            client,
                            keeper,
                            governor_address,
                            voter_storage_address,
