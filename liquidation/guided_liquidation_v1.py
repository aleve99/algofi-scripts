    ...
    for symbol, market in algofi_client.get_active_markets().items():
        if symbol not in DUPLICATE_ASSET_MARKETS:
            ...
            display_balance = round(
                balances[symbol] / 10 ** market.get_asset().get_underlying_decimals(), 6
            )
...
def display_target_data(
    algofi_client, target_address, target_state, target_totals, liquidator_balances
):
    print("{} CURRENT STATE".format(target_address))
    print(
        "User Max Borrow (USD): {}, User Total Borrow (USD): {}, Health Ratio: {}".format(
            round(target_totals["max_borrow_usd"], 2),
            round(target_totals["total_borrow_usd"], 2),
            round(
                target_totals["total_borrow_usd"] / target_totals["max_borrow_usd"], 10
            ),
        )
    )
    state_table = PrettyTable()
    ...
    for symbol, market in algofi_client.get_active_markets().items():
        ...
        display_borrow = round(
            target_state[symbol]["borrow_underlying"] / decimal_scale_factor, 6
        )
        display_collateral = round(
            target_state[symbol]["active_collateral_underlying"] / decimal_scale_factor,
            6,
        )
...
def load_borrowable_balances(algofi_client, address):
    ...
    for symbol, market in algofi_client.get_active_markets().items():
        if symbol not in DUPLICATE_ASSET_MARKETS:
            user_balances[symbol] = balances.get(
                market.get_asset().get_underlying_asset_id(), 0
            )
    return user_balances


def get_borrow_totals(algofi_client, user_state):
    totals = {"total_borrow_usd": 0, "max_borrow_usd": 0}
...
def get_max_liquidation_for_pair(
    algofi_client,
    repay_symbol,
    collateral_symbol,
    target_state,
    ...
    limit_2 = int(
        target_state[collateral_symbol]["active_collateral_underlying"]
        * (PARAMETER_SCALE_FACTOR / collateral_market.get_liquidation_incentive())
        * (prices[collateral_symbol] / prices[repay_symbol])
        * 10
        ** (
            repay_market.get_asset().get_underlying_decimals()
            - collateral_market.get_asset().get_underlying_decimals()
        )
    )
...
def execute_liquidation(
    ...
    while liquidation_pending:
        liquidate_txn = algofi_client.prepare_liquidate_transactions(
            target_address,
            repay_symbol,
            repay_amount,
            collateral_symbol,
    ...
    while burn_pending:
        ...
        amount_to_burn = algofi_client.get_user_balance(
            asset_id=algofi_client.get_market(collateral_symbol)
            .get_asset()
            .get_bank_asset_id(),
            address=liquidator_address,
        )
        burn_txn = algofi_client.prepare_burn_transactions(
            collateral_symbol, amount_to_burn, liquidator_address
        )
...
if __name__ == "__main__":
    ...
    while True:
        ...
        liquidator_balances = load_borrowable_balances(
            algofi_client, liquidator_address
        )
        ...
        repayable_symbols = [
            sym
            for sym in algofi_client.get_active_ordered_symbols()
            if (target_state[sym]["borrow_usd"] > 0 and liquidator_balances.get(sym, 0))
        ]
        repay_symbol = get_user_selection(
            "Enter repay symbol:",
            {i + 1: repayable_symbols[i] for i in range(len(repayable_symbols))},
        )
        collateral_symbols = [
            sym
            for sym in algofi_client.get_active_ordered_symbols()
            if target_state[sym]["active_collateral_usd"] > 0
        ]
        collateral_symbol = get_user_selection(
            "Enter seize symbol:",
            {i + 1: collateral_symbols[i] for i in range(len(collateral_symbols))},
        )

        max_liquidation = get_max_liquidation_for_pair(
            algofi_client,
            repay_symbol,
            collateral_symbol,
            target_state,
        ...
        max_liquidation_base = round(
            max_liquidation
            / 10
            ** algofi_client.get_market(repay_symbol)
            .get_asset()
            .get_underlying_decimals(),
            6,
        )
        repay_amount_base = get_user_numeric_value(
            "Enter amount to liquidate.",
            0,
            max_liquidation_base,
            max_liquidation_base * DEFAULT_TAKE_PERCENTAGE,
        )
        repay_amount = int(
            repay_amount_base
            * 10
