...
def process_storage_account_data(
    algofi_client,
    storage_account_data,
    decimal_scale_factors,
    market_app_ids,
    prices,
    exch_rates,
    underlying_borrowed_data,
    outstanding_borrow_shares_data,
    collateral_factors,
):
    # process data
    market_app_ids_set = set(market_app_ids)
    num_supported_markets = len(market_app_ids)
    market_app_to_symbols = dict(
        zip(market_app_ids, algofi_client.get_active_markets())
    )
    ...
    for app_data in local_state:
        ...
        if app_data["id"] in market_app_ids_set:
            borrow_token, borrow_usd, active_collateral_token, active_collateral_usd = (
                0,
                0,
                0,
                0,
            ...
            if "key-value" in app_data:
                ...
                borrow_shares = data.get(
                    b64encode(bytes(market_strings.user_borrow_shares, "utf-8")).decode(
                        "utf-8"
                    ),
                    0,
                )
                if borrow_shares:
                    borrow_token = (
                        (borrow_shares * underlying_borrowed_data[app_data["id"]])
                        / outstanding_borrow_shares_data[app_data["id"]]
                        / decimal_scale_factors[app_data["id"]]
                    )
                    borrow_usd = borrow_token * price
                    borrow += borrow_usd
                # get dollarized max borrow
                active_collateral_token = data.get(
                    b64encode(
                        bytes(market_strings.user_active_collateral, "utf-8")
                    ).decode("utf-8"),
                    0,
                )
                if active_collateral_token:
                    exch_rate = exch_rates[app_data["id"]]
                    active_collateral_token = (
                        active_collateral_token
                        / decimal_scale_factors[app_data["id"]]
                        * exch_rate
                    )
                    ...
                    max_borrow += (
                        active_collateral_usd * collateral_factors[app_data["id"]]
                    )
            market_counter += 1
            data_dict[market_app_to_symbols[app_data["id"]]] = {
                "collateral_token": active_collateral_token,
                "collateral_usd": active_collateral_usd,
...
def get_liquidation_data(algofi_client):
    ...
    bank_to_underlying_exchange_rates = [
        market.get_bank_to_underlying_exchange() for market in markets
    ]
    exch_rates = dict(
        zip(
            market_app_ids,
            scale_values(
                bank_to_underlying_exchange_rates, 1.0 / algofi_client.SCALE_FACTOR
            ),
        )
    )
    ...
    outstanding_borrow_shares = [
        market.get_outstanding_borrow_shares() for market in markets
    ]
    outstanding_borrow_shares_data = dict(
        zip(market_app_ids, outstanding_borrow_shares)
    )
    coll_factors = [market.get_collateral_factor() for market in markets]
    collateral_factors = dict(
        zip(
            market_app_ids,
            scale_values(coll_factors, 1.0 / algofi_client.PARAMETER_SCALE_FACTOR),
        )
    )
    ...
    for storage_account_data in storage_accounts:
        ...
        user_health_ratio_data[storage_account] = process_storage_account_data(
            algofi_client,
            storage_account_data,
            decimal_scale_factors,
            market_app_ids,
...
def process_liquidation_data(
    ...
    for user in liquidate_data:
        ...
        if (
            ...
            health_ratio = round(
                float(data["borrow"] / data["max_borrow"]), round_decimals
            )
            summary_dict["Health Ratio"].append(health_ratio)
            for symbol in algofi_client.get_active_markets():
                if (data[symbol]["borrow_usd"] > 0) or (
                    data[symbol]["collateral_usd"] > 0
                ):
                    drilldown_dict["Storage Account"].append(user)
                    drilldown_dict["Symbol"].append(symbol)
                    drilldown_dict["Collateral"].append(
                        round(float(data[symbol]["collateral_token"]), round_decimals)
                    )
                    drilldown_dict["Borrow"].append(
                        round(float(data[symbol]["borrow_token"]), round_decimals)
                    )
                    drilldown_dict["Collateral (USD)"].append(
                        round(float(data[symbol]["collateral_usd"]), round_decimals)
                    )
                    drilldown_dict["Borrow (USD)"].append(
                        round(float(data[symbol]["borrow_usd"]), round_decimals)
                    )
