...
class AlgofiUserState:
    ...
    def initialize_user_market_state(self, market_app_id, market):
        decimals = self.algofi_client.assets[self.algofi_client.lending.markets[market_app_id].underlying_asset_id].decimals
...
def get_liquidate_data(algofi_client):
    ...
    while next_page is not None:
        account_data = algofi_client.indexer.accounts(limit=1000, next_page=next_page, application_id=manager_app_id, exclude="assets")
                    ...
                    if "key-value" in state:
                        ...
                        if MARKET_STRINGS.user_borrow_shares in data:
                            ...
                            if market.borrow_share_circulation != 0:
                                borrow_underlying = (borrow_shares * market.underlying_borrowed) / market.borrow_share_circulation
                                scaled_borrow_usd = market.underlying_to_usd(borrow_underlying) * (market.borrow_factor / 1000)
                                user_state.update_user_market_state(market_app_id = market_app_id, borrow = borrow_underlying)
                        ...
                        if MARKET_STRINGS.user_active_b_asset_collateral in data:
                            collateral_b_asset_amount = data[MARKET_STRINGS.user_active_b_asset_collateral]
                            active_collateral_underlying = (collateral_b_asset_amount * market.get_underlying_supplied()) / market.b_asset_circulation
                            active_collateral_usd = market.underlying_to_usd(active_collateral_underlying)
                            user_state.update_user_market_state(market_app_id = market_app_id, collateral = active_collateral_underlying)
                            user_state.increment_max_borrow(active_collateral_usd * (market.collateral_factor) / 1000)
                if market_app_id == manager_app_id:
                    if "key-value" in state:
                        data = format_state_simple(state["key-value"])
                        if MANAGER_STRINGS.user_account in data:
                            unformatted_primary_address = data[MANAGER_STRINGS.user_account]
                            # set the primary address
                            user_state.set_primary_address(format_address_b32(unformatted_primary_address))
...
def process_liquidation_data(timestamp, liquidate_data, health_ratio_threshold, dollarized_borrow_threshold):
    round_decimals = 3
    ...
    for user_state in liquidate_data:
        ...
        if (borrow >= max_borrow*health_ratio_threshold) and (borrow >= dollarized_borrow_threshold) and (borrow != 0 and max_borrow != 0):
            ...
            summary_dict["Health Ratio"].append(round(user_state.health_ratio,round_decimals))
            ...
            for market_app_id in user_state.market_states.keys():
                ...
                if user_market_state.borrow_usd > 0 or user_market_state.collateral_usd > 0:
                    drilldown_dict["Storage Account"].append(user_state.storage_address)
                    drilldown_dict["Symbol"].append(user_state.market_states[market_app_id].market.name)
                    drilldown_dict["Collateral"].append(round(float(user_market_state.collateral),round_decimals))
                    drilldown_dict["Borrow"].append(round(float(user_market_state.borrow), round_decimals))
                    drilldown_dict["Collateral (USD)"].append(round(float(user_market_state.collateral_usd), round_decimals))
                    drilldown_dict["Borrow (USD)"].append(round(float(user_market_state.borrow_usd), round_decimals))
