...
if __name__ == '__main__':
    ...
    vault_accounts = algofi_client.get_storage_accounts(verbose=True)
    valgo_app_id = algofi_client.markets["vALGO"].market_app_id
    vault_accounts_filtered = []
    active_collateral = "dWFj"
    ...
    for vault_account in vault_accounts:
        ...
        for app_local_state in user_local_state:
            if app_local_state["id"] == valgo_app_id:
                fields = app_local_state.get("key-value", [])
                for field in fields:
                    if field["key"] == active_collateral:
    ...
    summary_df.to_csv(args.csv_fpath + "v1-vault-summary-%s.csv" % ts)